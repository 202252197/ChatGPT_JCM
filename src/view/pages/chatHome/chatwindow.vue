          ...
          decodedArray.forEach(decoded => {
            ...
            if(decoded!==""){
              if(decoded.trim()==="[DONE]"){
                ...
                if(JSON.parse(decoded).choices==null){
                  this.$message.error("请检查输入的Key是否正确或余额是否充足,没有key是获取不到模型列表的，请到设置中添加key");
                  return
                }
                if(type==="chat"){
    ...
    sendText() {
      ...
      if (this.inputMsg) {
        ...
        if (this.settingInfo.openProductionPicture) {
          ...
          const chatModels=["gpt-3.5-turbo","gpt-3.5-turbo-0301","GPT-4o","GPT-4o-mini"]
          if (chatModels.includes(this.frinedInfo.id)) {
            ...
            if(this.settingInfo.cutSetting===0){
              if(this.frinedInfo.id === "text-davinci-003" ){
                ...
                this.$message.error("默认关闭chatModels模型数组以外的模型聊天")
                this.$nextTick(() => {
                  this.acqStatus = true
                })
              }
